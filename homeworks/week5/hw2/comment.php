<div class='container commentarea'>
	...
	<div class='comment-display main_comment'>
		...
		?>
			<form class="comment__form adding" action="add_comment.php" method="POST">
				<div class="comment__form__icon marginbottomzero">
					<?php
						$stmt2 = $conn->prepare("SELECT * from users LEFT JOIN users_certificate ON users_certificate.username = users.username WHERE certificate = ?");
		...
		<?php
			...
			//每個主留言欄的迴圈裡面還得再抓副留言
			$stmt1 = $conn->prepare("SELECT * FROM comments LEFT JOIN users ON comments.user_id = users.id WHERE major = ? ORDER BY created_at DESC");
