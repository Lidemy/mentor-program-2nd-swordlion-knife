<?php
	...
	$stmt = $conn->prepare("SELECT * FROM swordlion_knife_subcomment WHERE major = ? ");
	$stmt->bind_param('s',$_POST['num']);
	$stmt->execute();
	$result = $stmt->get_result();
	$fetch = $result->fetch_assoc();

	if($fetch['content'] == $_POST['content']) {
		echo "<script>alert ('還是要改一下啦~~'); location.replace(document.referrer);</script>";
	} else {
		$stmt = $conn->prepare("UPDATE swordlion_knife_subcomment SET content = ? WHERE major = ? ");
		$stmt->bind_param("ss",$_POST['content'],$_POST['num']);
		$stmt->execute();
